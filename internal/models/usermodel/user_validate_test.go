...
var (
	...
	wrongEmail    = userfields.Email(amidstr.MakeString(userfields.EMAIL_MAX_LENGTH))
	wrongEmailDTO = usermodel.NewCreateUserDTO(maxName, maxSurname, fatherName, wrongEmail, roles)
)
...
func TestUnvalidEmail(t *testing.T) {
	exp := error(usererror.WRONG_EMAIL)
	act := wrongEmailDTO.Validate()
	assert.ErrorEqual(t, exp, act, "validate email failed with wrong input")

	wrongEmailDTO.Email = email
	exp = nil
	act = wrongEmailDTO.Validate()
	assert.ErrorEqual(t, exp, act, "validate email failed with right input")
}
